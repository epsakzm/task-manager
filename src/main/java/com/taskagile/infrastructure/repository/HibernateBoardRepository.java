...
public class HibernateBoardRepository extends HibernateSupport<Board> implements BoardRepository {
    ...
    protected HibernateBoardRepository(EntityManager entityManager) {
        super(entityManager);
    }
    ...
    public Board findById(BoardId boardId) {
        Query<Board> query = getSession().createQuery("SELECT b FROM Board b WHERE b.id = :id", Board.class);
